function [GLCMS,SI] = graycomatrix3D(varargin)
...
if NL ~= 0
  ...
  % neighbor.
  s = size(I);
  [r,c,h] = meshgrid(1:s(1),1:s(2),1:s(3));
  r = r(:);
  c = c(:);
  h = h(:);

  % Compute GLCMS
  GLCMS = zeros(NL,NL,NL,numOffsets);
  for k = 1 : numOffsets
    GLCMS(:,:,:,k) = computeGLCM(r,c,h,Offset(k,:),SI,NL);
  ...
  GLCMS = zeros(0,0,0,numOffsets);
end

%-----------------------------------------------------------------------------
function oneGLCM = computeGLCM(r,c,h,offset,si,nl)
...
[nRow nCol nHei] = size(si);
% Determine locations where subscripts outside the image boundary
outsideBounds = find(c2 < 1 | c2 > nCol | r2 < 1 | r2 > nRow | h2 < 1 | h2 > nHei);

% Create vector containing si(r1,c1,h1)
v1 = shiftdim(si,1);
v1 = v1(:);
v1(outsideBounds) = [];

% Create vector containing si(r2,c2,h2). Not using sub2ind for performance
% reasons
r2(outsideBounds) = []; 
c2(outsideBounds) = []; 
h2(outsideBounds) = []; 
Index = r2 + h2*nRow+(c2 - 1)*nRow;
v2 = si(Index);
Index2= c2 + h2*nCol+(r2 - 1)*nCol;
v3 = si(Index2);
...
if isempty(Ind)
    ...
    oneGLCM = accumarray(Ind, 1, [nl nl nl]);
end
