...
% funtion:sort the data of Network

clc
clear all;
close all;

pathname='E:\ADHKUSEGresult\Poutputs';           %^^change this to make analysis among NC or PSD/or AD
cd(pathname);
% cd('AllAtlasResults');
...
filenames=dircell(:,1);
savename='Network_analysis_FN';%name of save folder
mkdir(savename);

for i=3:19                     %3-14(NC)or3-18(PSD)depends on the number of folders^^change this to choice
    cd(char(filenames(i)));
    cd('Network');
    cd('Deterministic');
    ...
    for j=1:length(files)
        ...
        if isempty(ind{1})==0               %test if it is FA
            copyfile(char(files(j)),'E:\ADHKUSEGresult\Poutputs\Network_analysis_FN');
